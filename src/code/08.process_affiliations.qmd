...
dates = pubs_authors |>
    ...
    group_by(uid, authorships_id, authorships_affiliations_display_name, authorships_affiliations_id) |>
    summarize(
        uid = first(uid),
...
format_institutions = function(d) {
  d |>
    distinct(uid, year, authorships_affiliations_display_name) |>
    filter(year > 15) |>
    arrange(year) |>
    group_by(uid, year) |>
    reframe(
      affiliations = paste(
        unique(authorships_affiliations_display_name), collapse = "; "
      )
    ) |>
    pivot_wider(
...
```{r}
timepoints <- c("2015-09-01", "2019-09-01", "2022-12-19", "2024-04-19", "2025-10-02")
tp <- ymd(timepoints)
...
d = dates |>
  ...
  filter(in_range) |>
  distinct(uid, authorships_display_name, authorships_id, authorships_display_name, authorships_affiliations_display_name, year) |>
  filter(!is.na(authorships_affiliations_display_name)) |>
  format_institutions()
