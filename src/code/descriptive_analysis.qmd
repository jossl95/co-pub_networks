...
```{r}
#| warning: false
#| fig.cap: The number of publications per year of our sample
...
```{r}
#| warning: false
#| fig.cap: The number of valid publications per year of our sample
...
dems_long = dems |>
  pivot_longer(
    ...
    cols = matches("^(email_adres|universiteit|functie|discipline)_\\d{2}$"),
  ...
  select(-year2) |>
  arrange(uid, year) |>
  mutate(universiteit = na_if(universiteit, "")) |>
  separate_rows(universiteit, sep = "\\s*/\\s*")
```
