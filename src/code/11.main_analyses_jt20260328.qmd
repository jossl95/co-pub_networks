---
title: Main Analysis
date: today

format:
  html:
    code-links:
      - href: 0a.custom_functions.html
        icon: file-code
        text: Custom Functions

execute: 
  eval: false
---


## Getting Started

```{r}
#| warning: false
#| output: false
#| echo: false
BASEDIR = stringr::str_replace_all(getwd(), '/results', '')
setwd(BASEDIR)
```

```{r}
#| warning: false
#| output: false
# load custom functions
source("src/utils/custom_functions.r")

# clear the global environment and set dependencies
.clear_global_environment()
.load_quarto_dependencies()

BASEDIR = stringr::str_replace_all(getwd(), '/results', '')
setwd(BASEDIR)
```

```{r}
#| warning: false
#| output: false

# load and activate packages
library(tidyverse)
library(RSiena)
library(igraph)
library(dplyr)
library(tibble)
```

## Data

```{r}
data <- freadRDS2('data', location = "./data/analysis/")
topics <-freadRDS2('topics', location = './data/analysis/')
```

## Functions 

```{r}
# step 2: prune works for selection
make_adjacency_matrix <- function(
    data, uids, type = 'first', min_year = 2020, max_year = 2026, weighted = FALSE
  ) {

  # make edgelist from works
  edges <- data[['works']][uids] |> 
    purrr::keep(~ nrow(.x) > 0) |>
    bind_rows()
  
  if (inherits(min_year, "Date")) {
    edges <- edges |> filter(publication_date >= min_year, publication_date <= max_year)
  } else {
    edges <- edges |> filter(publication_year >= min_year, publication_year <= max_year)
  }
  edges <- edges |> 
    select(uid, work_id, authorships) |>
    rename(e_uid = uid) |>
    distinct(work_id, .keep_all = TRUE) |>
    unnest(authorships) 

  if (type != 'all') {
    edges <- edges |>
      group_by(work_id) |>
      mutate(
        e_uid = uid[author_position == type][1]
      ) |> ungroup() |>
      filter(!is.na(e_uid), e_uid != uid) |>
      select(work_id, e_uid, uid)
  } else {
    edges <- edges |> 
      group_by(work_id) |>
      reframe(
        tidyr::crossing(
          e_uid = uid,
          uid   = uid
        )
      ) |>
      ungroup() |>
      filter(!is.na(e_uid), e_uid != uid) |>
      select(work_id, e_uid, uid)
  }

  # calculate the number of edges between ego and alters.
  edge_counts <- edges |> count(e_uid, uid, name="w")

  # convert counts to incidences
  if (!weighted) {
    edge_counts <- edge_counts %>% mutate(w = 1L)
  }

  # all nodes appearing as ego or alter
  nodes <- sort(unique(c(edge_counts$e_uid, edge_counts$uid)))
  
  # complete matrix grid and pivot to wide
  adj <- edge_counts %>%
    rename(from = e_uid, to = uid) %>%
    complete(from = nodes, to = nodes, fill = list(w = 0)) %>%
    pivot_wider(names_from = to, values_from = w) %>%
    arrange(from)
  
  # convert to matrix and set rownames
  mat <- adj %>% as.data.frame()
  
  row_ids <- mat$from
  mat$from <- NULL
  
  mat <- as.matrix(mat)
  rownames(mat) <- row_ids
  diag(mat) <- 0

  # make sure the matrices are consistent across waves
  all_uids <- as.character(uids)

  # indices of existing rows/cols in desired order
  ri <- match(all_uids, rownames(mat))
  ci <- match(all_uids, colnames(mat))

  # start with full NA matrix
  mat_full <- matrix(0L, nrow = length(all_uids), ncol = length(all_uids),
                    dimnames = list(all_uids, all_uids))

  # copy overlap block
  mat_full[!is.na(ri), !is.na(ci)] <- mat[ri[!is.na(ri)], ci[!is.na(ci)], drop = FALSE]
  mat <- mat_full

  mat
} 


fcolnet2 <- function(
    data,
    university = NULL,
    position   = NULL,
    discipline = NULL,
    type = NULL,
    waves = list(c(2019, 2022), c(2023, 2024), c(2025, 2026))
  ){

  valid_disciplines  <- c("Sociology", "Political Sciences")
  valid_universities <- c("UVA","EUR","UL","VU","UVG","UU","RU","UVT")
  valid_positions    <- c("Associate Professor","PhD Candidate","Full Professor",
                          "Researcher or Lecturer","Assistant Professor")
  valid_dates        <- list(c(ymd('20201219'), ymd('20221219')), 
                             c(ymd('20221220'), ymd('20240419')),
                             c(ymd('20240420'), ymd('20260201')))

  # helper: set default + validate
  set_and_check <- function(x, valid, name = deparse(substitute(x))) {
    x <- if (is.null(x)) valid else x
    bad <- setdiff(unique(na.omit(x)), valid)
    if (length(bad)) stop("Invalid ", name, ": ", paste(bad, collapse = ", "), call. = FALSE)
    x
  }

  # set and check values
  discipline <- set_and_check(discipline, valid_disciplines,  "discipline")
  university <- set_and_check(university, valid_universities, "university")
  position   <- set_and_check(position,   valid_positions,    "position")
  waves      <- if(is.null(waves)) valid_dates else waves
  min_year   <- min(unlist(waves))
  max_year   <- max(unlist(waves))

  # step 1: make selection of nodes
  authors <- data[["demographics"]] |>
    filter(
      if_any(c(university_22, university_24, university_25),
            ~ .x |>
              toupper() |>
              str_detect(paste(university, collapse = "|"))),
      if_any(c(discipline_22, discipline_24, discipline_25),
            ~ .x %in% discipline),
      if_any(c(position_22, position_24, position_25),
            ~ .x %in% position)
    ) |>
    arrange(uid)
  uids <- authors |> pull(uid) |> sort()
  
  # step 3: create empty matrixes (wave, i, j)
  nwaves <- length(waves)
  nets <- list()
  if (is.null(type)){
    types = c('first', 'last', 'all')
    for (t in types){
      for (w in 1:length(waves)){
        nets[[t]][[w]] <- make_adjacency_matrix(
          data, uids, type = t, min_year = waves[[w]][1], max_year = waves[[w]][2])
      }
    }
  } else {
    for (w in 1:length(waves)){
      nets[[w]] <- make_adjacency_matrix(
        data, uids, type, min_year = waves[[w]][1], max_year = waves[[w]][2])
    }
  }

  # step 4: fill nets
  output <- list(
    data = authors,
    nets = nets
  )

  return(output)
}
```

```{r}
harmonize_covariates <- function(data, what = 'data'){
  uni_levels = c("EUR", "RU" , "UL", "UU", "UVA", "UVG", "UVT", "VU")

  data[[what]] = data[[what]] |>
    mutate(
      female = as.integer(gender == 'female'),
      soc = pmap_chr(
        list(discipline_22, discipline_24, discipline_25),
        ~ unique(na.omit(c(...)))[1]
      ),
      soc = as.integer(soc == 'Sociology')
    ) |>
    rename_with(
      ~ .x |> 
        str_replace('university_', 'uni') |>
        str_replace('_first$', 'p')
    ) |>
    mutate(
      across(
        starts_with('uni') & ends_with('p'),
        ~ factor(tolower(.x), 
        levels = tolower(uni_levels), 
        ordered = TRUE)
      )
    )

  return(data)
}
```

```{r}
mask_structural_missing <- function(colnet, include_ties = FALSE, value = 10){
  for (type in names(colnet[['nets']])){
    for (i in 1:length(colnet[['nets']][[type]])){
      net <- colnet[['nets']][[type]][[i]]
      
      no_ties <- rowSums(net) == 0 * include_ties
      abscent <- colnet[['data']] |> 
        column_to_rownames('uid') |>
        pull(paste0("uni", c(22, 24, 25)[i])) |> 
        is.na()

      exclude <- no_ties & abscent
      net[exclude, ] <- value
      net[ ,exclude] <- value

      colnet[['nets']][[type]][[i]] <- net
    }
  }

  return(colnet)
}
```

## RSiena preparation

### Set selection

<!-- Jos, ik blijf de ruwe data vreemd vinden. Waarom heb ik geen UVG affiliatie in 2025, waarom is Peer er uit in 2025, waarom zit Wout Ultee er enkel in in 2025? 
Ik vind ruis in de data prima, maar ik wil wel weten waar het vandaan komt. 
De fout bij mij is echt een fout van de SA die de ruwe data heeft verzameld. 
De fouten bij Peer en Wout kunnen mogelijk komen door foute op de websites van de uni. 
--> 

```{r}
# uni_levels = tolower(c("EUR", "RU" , "UL", "UU", "UVA", "UVG", "UVT", "VU"))
uni_levels = tolower(c(
  "RU", "UU", "UVT"
))

colnet <- fcolnet2(data, 
    discipline = c("Sociology", "Political Sciences"),
    university = toupper(uni_levels),
    type = NULL,
    waves = NULL
  ) |>
  harmonize_covariates(what='data') |>
  mask_structural_missing()
```

### Dependent variable 
```{r}
wave1 = colnet[['nets']][['first']][[1]]
wave2 = colnet[['nets']][['first']][[2]]
wave3 = colnet[['nets']][['first']][[3]]

wave1l = colnet[['nets']][['last']][[1]]
wave2l = colnet[['nets']][['last']][[2]]
wave3l = colnet[['nets']][['last']][[3]]

wave1a = colnet[['nets']][['all']][[1]]
wave2a = colnet[['nets']][['all']][[2]]
wave3a = colnet[['nets']][['all']][[3]]


net_array <- array(
    data = c(wave1, wave2, wave3),
    dim = c(dim(wave1), 3)
)
first <- sienaDependent(net_array, )

net_array <- array(
    data = c(wave1l, wave2l, wave3l),
    dim = c(dim(wave1l), 3)
)
last <- sienaDependent(net_array, )

net_array <- array(
    data = c(wave1a, wave2a, wave3a),
    dim = c(dim(wave1l), 3)
)
all <- sienaDependent(net_array, )


```

### time constant covariates

- gender
- soc
- mig (NL, M4, other)
- lft

```{r}
gender <- coCovar(colnet[['data']]$female)
#we observe no switches between disciplines
soc <- coCovar(colnet[['data']]$soc)

# set migrant constant covars
migrant_groups = c('NL', 'M4', 'other')
mig <- migrant_groups[-1] |>
  set_names() |>
  map(function(group){
    val <- as.integer(colnet[['data']]$migrant == group)

    coCovar(val)
  })

migrantf <- as.numeric(as.factor(colnet[['data']]$migrant))
migrant <- coCovar(migrantf)

lft <- coCovar(2027 - replace_na(colnet[['data']]$career_start, 2027))


```

### time varying covariates

- uni  
- funcs
- intdis: based on subfield
- i-index

<!-- ik wil de postdocs bij de PhD en nu ik resultaten zie, wrs ook de researchers en lectureqrs, maar dat kan heel makkelijk. fijn. --> 
<!--# I still miss I-index --> 
<!-- we should probably set missing values for these covariates if not present --> 

```{r}
# set values for time varying universities
uni = colnet[['data']] |>
  select(uni22, uni24, uni25)

uni <- uni_levels[-1] |> 
  set_names() |>
  toupper() |>
  map(function(level) {
    mat <- uni |>
      transmute(
        u22 = as.integer(str_detect(uni22, level)), 
        u24 = as.integer(str_detect(uni24, level)),
        u25 = as.integer(str_detect(uni25, level)) # excluded for model specification
      ) |>
      as.matrix()

    varCovar(mat[, c(1, 2)], centered = FALSE)
  })

# functions
#combine postdocs with phds
colnet[['data']] <- colnet[['data']] |> 
  mutate(
    position_22_b = ifelse(position_22 == "Researcher or Lecturer" & position2_22 == "Postdoctoral Researcher", "PhD Candidate", position_22), 
    position_24_b = ifelse(position_24 == "Researcher or Lecturer" & position2_24 == "Postdoctoral Researcher", "PhD Candidate", position_24), 
    position_25_b = ifelse(position_25 == "Researcher or Lecturer" & position2_25 == "Postdoctoral Researcher", "PhD Candidate", position_25)) 


funcs = colnet[['data']] |> 
  select(position_22_b, position_24_b, position_25_b)
funcs_levels = c(
  "PhD Candidate", "Researcher or Lecturer", 
  "Assistant Professor", "Associate Professor", 
  "Full Professor"
)

funcs = funcs_levels[-1] |>
  set_names() |>
  map(function(level){
    mat <- funcs |>
      transmute(
        f22 = as.integer(position_22_b == level),
        f24 = as.integer(position_24_b == level), #there was a type :-(
        f25 = as.integer(position_25_b == level) # excluded for model specification
      ) |>
      as.matrix()
    
    varCovar(mat[, c(1, 2)], centered = FALSE)
  })

funcs_di = colnet[['data']] |> 
  select(position_22_b, position_24_b, position_25_b)
funcs_levels = c(
  "PhD Candidate", "Researcher or Lecturer", 
  "Assistant Professor", "Associate Professor", 
  "Full Professor"
)

#UD, UHD, HL versus other
funcs_di <- funcs_di |>
  mutate(
    f22_di = as.integer(position_22_b %in% funcs_levels[3:5]),
    f24_di = as.integer(position_24_b %in% funcs_levels[3:5]), 
    f25_di = as.integer(position_25_b %in% funcs_levels[3:5]), 
    .keep = "none"
  ) |>
  as.matrix()
    
funcs_di <- varCovar(funcs_di[, c(1, 2)], centered = FALSE)

i_index <- colnet[['data']] |> 
  select(i10_index_21, i10_index_23, i10_index_25) |>
  mutate(
    i10_index_21 = rank(ifelse(is.na(i10_index_21),0, i10_index_21), ties.method = "average"), 
    i10_index_23 = rank(ifelse(is.na(i10_index_23),0, i10_index_23), ties.method = "average"),     
    i10_index_25 = rank(ifelse(is.na(i10_index_25),0, i10_index_25), ties.method = "average"),
    i10_index_21 = i10_index_21 / max(i10_index_21),
    i10_index_23 = i10_index_23 / max(i10_index_23),
    i10_index_25 = i10_index_25 / max(i10_index_25)
    ) |>
  as.matrix()

i10 <- varCovar(i_index[, c(1, 2)])




  
add_interdisciplinarity = function(colnet, topics){
  uids = colnet$data |> pull(uid) |> unique() |> sort()
  intdis <- topics[["intdis"]][["subfield"]]

  colnet[["intdis"]] <- {
    out <- matrix(
      NA_real_,
      nrow = length(uids),
      ncol = ncol(intdis),
      dimnames = list(uids, colnames(intdis))
    )

    common_uids <- intersect(uids, rownames(intdis))
    out[common_uids, ] <- intdis[common_uids, , drop = FALSE]
    out
  }
  
  return(colnet)
}

colnet <- colnet |>
  add_interdisciplinarity(topics) 

intdis <- colnet[['intdis']][,1:2] |> varCovar(centered = TRUE)

```

### time varying dyadic covars

- dissim: based on subfield
- distan: travel time?? 
- sameuni


```{r}
add_dissimilarity <- function(colnet, topics) {
  uids = colnet$data |> pull(uid) |> unique() |> sort()

  colnet[["dissim"]] <- purrr::map(
    topics[["dissim"]][["subfield"]],
    \(mat) {
      common_uids <- Reduce(intersect, list(uids, rownames(mat), colnames(mat)))

      out <- matrix(
        NA_real_,
        nrow = length(uids),
        ncol = length(uids),
        dimnames = list(uids, uids)
      )

      out[common_uids, common_uids] <- mat[common_uids, common_uids, drop = FALSE]
      out
    }
  )

  return(colnet)
}

colnet <- colnet |>
  add_dissimilarity(topics)

dissim <- colnet[['dissim']][c('2022', '2024')] |> simplify2array() |> varDyadCovar(centered = TRUE)

add_distances <- function(colnet, data) {
  uids = colnet$data |> pull(uid) |> unique() |> sort()

  colnet[["distances"]] <- purrr::map(
    data[["distances"]],
    \(mat) {
      common_uids <- Reduce(intersect, list(uids, rownames(mat), colnames(mat)))

      out <- matrix(
        NA_real_,
        nrow = length(uids),
        ncol = length(uids),
        dimnames = list(uids, uids)
      )

      out[common_uids, common_uids] <- mat[common_uids, common_uids, drop = FALSE]
      out
    }
  )

  return(colnet)
}

colnet <- colnet |>
  add_distances(data)

distan = colnet[['distances']][c('2022', '2024')] |> simplify2array() |> varDyadCovar(centered = TRUE)



```

```{r}
fhomomat <- function(x, y) {
    xmat <- matrix(x, nrow = length(x), ncol = length(x))
    ymat <- matrix(y, nrow = length(y), ncol = length(y))
    xmatt <- t(xmat)
    ymatt <- t(ymat)
    xhomo <-  (
      (!is.na(xmat) & !is.na(xmatt) & (xmat == xmatt)) | 
      (!is.na(ymat) & !is.na(ymatt) & (ymat == ymatt)) | 
      (!is.na(xmat) & !is.na(ymatt) & (xmat == ymatt))
    )
    return(xhomo)
}

unis <- colnet[['data']] |>
  left_join(data[['demographics']]) |> 
  select(uid, university_22, university_24, university_25) |>
  mutate(
    uni22_f = sapply(strsplit(university_22, '/'), `[`, 1),
    uni22_s = sapply(strsplit(university_22, '/'), `[`, 2),
    uni24_f = sapply(strsplit(university_24, '/'), `[`, 1),
    uni24_s = sapply(strsplit(university_24, '/'), `[`, 2),
    uni25_f = sapply(strsplit(university_25, '/'), `[`, 1),
    uni25_s = sapply(strsplit(university_25, '/'), `[`, 2)
         )

sameuni <- list()

sameuni[[1]] <- (fhomomat(unis$uni22_s, unis$uni22_f)) * 1
sameuni[[2]] <- (fhomomat(unis$uni24_s, unis$uni24_f)) * 1
#sameuni[[3]] <- (fhomomat(unis$uni25_s, unis$uni25_f)) * 1

sameuni <-  sameuni |> simplify2array() |> varDyadCovar()


```
### combine data into mydata

```{r}
# combine variables into Rsiena data object
mydata <- sienaDataCreate(
  first, 
  gender, soc, mig$M4, mig$other, migrant, lft,
  uni$uu, uni$uvt,
  i10,
  funcs[[1]], funcs[[2]], funcs[[3]], funcs[[4]], funcs_di, intdis, 
  dissim, distan, sameuni
)

mydatal <- sienaDataCreate(
  last, 
  gender, soc, mig$M4, mig$other, migrant, lft,
  uni$uu, uni$uvt,
  i10,
  funcs[[1]], funcs[[2]], funcs[[3]], funcs[[4]], funcs_di, intdis, 
  dissim, distan, sameuni
)

mydataa <- sienaDataCreate(
  all, 
  gender, soc, mig$M4, mig$other, migrant, lft,
  uni$uu, uni$uvt,
  i10,
  funcs[[1]], funcs[[2]], funcs[[3]], funcs[[4]], funcs_di, intdis, 
  dissim, distan, sameuni
)
```

## RSiena analysis

### print01report
```{r}
setwd(BASEDIR)
if(!dir.exists("results")) dir.create("results")
setwd(paste0(BASEDIR, '/results'))
print01Report(mydata, modelname = "./init")

# model instability: perhaps throwing cases withh full missingness out

```

### set algortithm
```{r}
myAlgorithm <- sienaAlgorithmCreate(
  projname = "init",
  # n3 = 3000 # for publication ready results
)

myAlgorithm_un <- sienaAlgorithmCreate(
  projname = "init",
  modelType = c(all=3)
  # n3 = 3000 # for publication ready results
)

```
### first author perspective

```{r}
myeff <- getEffects(mydata)
myeff <- includeEffects(myeff, density, recip, inPopSqrt, outActSqrt,gwespFF, name='first')
#myeff <- includeEffects(myeff, outIso, name='first')
myeff <- includeEffects(myeff, egoX, interaction1="gender", name='first')
myeff <- includeEffects(myeff, altX, interaction1="gender", name='first')
myeff <- includeEffects(myeff, sameX, interaction1="gender", name='first')
myeff <- includeEffects(myeff, egoX, interaction1="funcs_di", name='first')
myeff <- includeEffects(myeff, altX, interaction1="funcs_di", name='first')
myeff <- includeEffects(myeff, egoX, interaction1="soc", name='first')
myeff <- includeEffects(myeff, altX, interaction1="soc", name='first')
myeff <- includeEffects(myeff, sameX, interaction1="soc", name='first')
myeff <- includeEffects(myeff, egoX, interaction1="mig$M4", name='first')
myeff <- includeEffects(myeff, altX, interaction1="mig$M4", name='first')
myeff <- includeEffects(myeff, egoX, interaction1="mig$other", name='first')
myeff <- includeEffects(myeff, altX, interaction1="mig$other", name='first')
myeff <- includeEffects(myeff, sameX, interaction1="migrant", name='first')
myeff <- includeEffects(myeff, egoX, interaction1="i10", name='first')
myeff <- includeEffects(myeff, altX, interaction1="i10", name='first')
myeff <- includeEffects(myeff, egoX, interaction1="intdis", name='first')
myeff <- includeEffects(myeff, altX, interaction1="intdis", name='first')
myeff <- includeEffects(myeff, absDiffX, interaction1="i10", name='first')
myeff <- includeEffects(myeff, X, interaction1="dissim", name='first')
myeff <- includeEffects(myeff, X, interaction1="sameuni", name='first')


# myeff <- includeEffects(myeff, egoX, interaction1="funcs[[2]]", name='first')
# myeff <- includeEffects(myeff, egoX, interaction1="funcs[[3]]", name='first')
# myeff <- includeEffects(myeff, egoX, interaction1="funcs[[4]]", name='first')
# myeff <- includeEffects(myeff, altX, interaction1="funcs[[1]]", name='first')
# myeff <- includeEffects(myeff, altX, interaction1="funcs[[2]]", name='first')
# myeff <- includeEffects(myeff, altX, interaction1="funcs[[3]]", name='first')
# myeff <- includeEffects(myeff, altX, interaction1="funcs[[4]]", name='first')
# 
#myeff <- includeInteraction(myeff, inPopSqrt, altX, interaction1=c("", "gender"))
#does not work, but don't know why


# myeff <- includeInteraction(myeff, inPopSqrt, altX, interaction1=c("", "gender"))
# myeff <- includeInteraction(myeff, altX, altX, interaction1=c("funcs[[4]]", "gender"))

# estimate model
ansm1 <- siena07(
  myAlgorithm, 
  data = mydata, 
  effects = myeff, 
  returnDeps = TRUE
)
ansm1
```

### last author perspective

see effect 138 altdist2!! social capital effects

```{r}

myeff <- getEffects(mydatal)

myeff <- includeEffects(myeff, outRateLog, name='last')

myeff <- includeEffects(myeff, density, recip, inPopSqrt, outActSqrt, gwespFF, cycle3, name='last')
#myeff <- includeEffects(myeff, outIso, name='last')
myeff <- includeEffects(myeff, egoX, interaction1="gender", name='last')
myeff <- includeEffects(myeff, altX, interaction1="gender", name='last')
myeff <- includeEffects(myeff, sameX, interaction1="gender", name='last')
myeff <- includeEffects(myeff, egoX, interaction1="funcs_di", name='last')
myeff <- includeEffects(myeff, altX, interaction1="funcs_di", name='last')
myeff <- includeEffects(myeff, egoX, interaction1="soc", name='last')
myeff <- includeEffects(myeff, altX, interaction1="soc", name='last')
myeff <- includeEffects(myeff, sameX, interaction1="soc", name='last')
myeff <- includeEffects(myeff, egoX, interaction1="mig$M4", name='last')
myeff <- includeEffects(myeff, altX, interaction1="mig$M4", name='last')
myeff <- includeEffects(myeff, egoX, interaction1="mig$other", name='last')
myeff <- includeEffects(myeff, altX, interaction1="mig$other", name='last')
myeff <- includeEffects(myeff, sameX, interaction1="migrant", name='last')
myeff <- includeEffects(myeff, egoX, interaction1="i10", name='last')
myeff <- includeEffects(myeff, altX, interaction1="i10", name='last')
myeff <- includeEffects(myeff, egoX, interaction1="intdis", name='last')
myeff <- includeEffects(myeff, altX, interaction1="intdis", name='last')
myeff <- includeEffects(myeff, absDiffX, interaction1="i10", name='last')
myeff <- includeEffects(myeff, absDiffX, interaction1="lft", name='last')
myeff <- includeEffects(myeff, X, interaction1="dissim", name='last')
myeff <- includeEffects(myeff, X, interaction1="sameuni", name='last')

# estimate model
ansm1l <- siena07(
  myAlgorithm, 
  data = mydatal, 
  effects = myeff, 
  returnDeps = FALSE,
  useCluster = TRUE, 
  nbrNodes = 20
)
ansm1l


```


### all author perspective

```{r}
myeff <- getEffects(mydataa)

myeff <- includeEffects(myeff, density, degPlus, parm=2, name='all')
myeff <- includeEffects(myeff, transTriads, parm=2, name='all')

myeff <- includeEffects(myeff, egoX, interaction1="gender", name='all')
#myeff <- includeEffects(myeff, altX, interaction1="gender", name='all')
myeff <- includeEffects(myeff, sameX, interaction1="gender", name='all')
#myeff <- includeEffects(myeff, egoX, interaction1="funcs_di", name='all')
#myeff <- includeEffects(myeff, altX, interaction1="funcs_di", name='all')
#myeff <- includeEffects(myeff, egoX, interaction1="soc", name='all')
#myeff <- includeEffects(myeff, altX, interaction1="soc", name='all')
myeff <- includeEffects(myeff, sameX, interaction1="soc", name='all')
#myeff <- includeEffects(myeff, egoX, interaction1="mig$M4", name='all')
#myeff <- includeEffects(myeff, altX, interaction1="mig$M4", name='all')
#myeff <- includeEffects(myeff, egoX, interaction1="mig$other", name='all')
#myeff <- includeEffects(myeff, altX, interaction1="mig$other", name='all')
myeff <- includeEffects(myeff, sameX, interaction1="migrant", name='all')
#myeff <- includeEffects(myeff, egoX, interaction1="i10", name='all')
#myeff <- includeEffects(myeff, altX, interaction1="i10", name='all')
#myeff <- includeEffects(myeff, egoX, interaction1="intdis", name='all')
#myeff <- includeEffects(myeff, altX, interaction1="intdis", name='all')
myeff <- includeEffects(myeff, absDiffX, interaction1="i10", name='all')
myeff <- includeEffects(myeff, absDiffX, interaction1="lft", name='all')
myeff <- includeEffects(myeff, X, interaction1="dissim", name='all')
myeff <- includeEffects(myeff, X, interaction1="sameuni", name='all')

# estimate model
ansm1a <- siena07(
  myAlgorithm_un, 
  data = mydataa, 
  effects = myeff, 
  useCluster = TRUE, 
  nbrNodes = 20,
  returnDeps = FALSE
)
ansm1a


```






```{r}
# specify model
library(glue)

includeDefaultEffects <- function(myeff){
  myeff <- includeEffects(myeff, density, recip, inPop) #name='first'
  myeff <- includeEffects(myeff, density, recip, inPop, name='last')
  # myeff <- includeEffects(myeff, antiInIso) # does not work in later models

  # # add node characteristics
  # myeff <- includeEffects(myeff, egoX, interaction1 = "soc")
  # myeff <- includeEffects(myeff, egoX, interaction1 = "soc", name='last')
  # # sameX soc collinear with university (uu has no pol-sci)
  for (name in names(uni)){
    myeff <- includeEffects(myeff, egoX, interaction1 = glue("uni${name}"))
    myeff <- includeEffects(myeff, egoX, interaction1 = glue("uni${name}"), name='last')
    # myeff <- includeEffects(myeff, simX, interaction1 = glue("uni${name}"))
  }

  return(myeff)
}

myeff <- getEffects(mydata)
myeff <- includeDefaultEffects(myeff)

myAlgorithm <- sienaAlgorithmCreate(
  projname = "init",
  # n3 = 3000 # for publication ready results
)
# estimate model
ansm1 <- siena07(
  myAlgorithm, 
  data = mydata, 
  effects = myeff, 
  returnDeps = TRUE
)

ansm1
save(ansm1, file = './ansm1')
```

```{r}
# myeff <- getEffects(mydata)
# myeff <- includeDefaultEffects(myeff)
myeff <- includeEffects(myeff, egoX, interaction1 = "gender") # altX, sameX
myeff <- includeEffects(myeff, altX, interaction1 = "gender") 
myeff <- includeEffects(myeff, sameX, interaction1 = "gender") 
myeff <- includeEffects(myeff, egoX, interaction1 = "gender", name='last') # altX, sameX
myeff <- includeEffects(myeff, altX, interaction1 = "gender", name='last') 
myeff <- includeEffects(myeff, sameX, interaction1 = "gender", name='last') 
# too few minority in sample leave out for now.
# myeff <- includeEffects(myeff, egoX, altX, simX, interaction1 = 'mig$M4') # altX, sameX
# myeff <- includeEffects(myeff, egoX, altX, simX, interaction1 = 'mig$other') # altX, sameX

ansm2 <- siena07(
  myAlgorithm, 
  data = mydata, 
  effects = myeff,
  returnDeps = TRUE
)

save(ansm2, file = './ansm2')
ansm2
```

```{r}
# myeff <- getEffects(mydata)
# myeff <- includeDefaultEffects(myeff)
myeff <- includeEffects(myeff, egoX, altX, interaction1 = 'funcs[[1]]')
myeff <- includeEffects(myeff, egoX, altX, interaction1 = 'funcs[[2]]')
myeff <- includeEffects(myeff, egoX, altX, interaction1 = 'funcs[[3]]')
myeff <- includeEffects(myeff, egoX, altX, interaction1 = 'funcs[[4]]')
myeff <- includeEffects(myeff, egoX, altX, interaction1 = 'funcs[[1]]', name='last')
myeff <- includeEffects(myeff, egoX, altX, interaction1 = 'funcs[[2]]', name='last')
myeff <- includeEffects(myeff, egoX, altX, interaction1 = 'funcs[[3]]', name='last')
myeff <- includeEffects(myeff, egoX, altX, interaction1 = 'funcs[[4]]', name='last')

ansm3 <- siena07(
  myAlgorithm, 
  data = mydata, 
  effects = myeff, 
  returnDeps = TRUE,
)

save(ansm3, file = './ansm3')

ansm3
```
```{r}
# myeff <- getEffects(mydata)
# myeff <- includeDefaultEffects(myeff)
myeff <- includeEffects(myeff, egoX, altX, interaction1 = 'intdis') # simX does noet work
myeff <- includeEffects(myeff, egoX, altX, interaction1 = 'intdis', name='last') # simX does noet work
myeff <- includeEffects(myeff, X, interaction1 = 'dissim') 
myeff <- includeEffects(myeff, X, interaction1 = 'dissim', name='last') 
myeff <- includeEffects(myeff, X, interaction1 = "distan")
myeff <- includeEffects(myeff, X, interaction1 = "distan", name='last')

ansm4 <- siena07(
  myAlgorithm, 
  data = mydata, 
  effects = myeff, 
  returnDeps = TRUE,
)

save(ansm4, file = './ansm4')
ansm4
```

```{r}
tidy_rsiena <- function(ans) {
  # extract effects
  effects = tibble(
      depvar = ans$effects$name,
      effect = ans$effects$effectName,
      short  = ans$effects$shortName,
      interaction1 = ans$effects$interaction1,
      interaction2 = ans$effects$interaction2,
      include = ans$effects$include,
      estimate = ans$theta,
      se = ans$se,
      conv_t = ans$tconv
    ) |>
    filter(include) 
  
  # # extract rate parameters
  # rate = tibble(
  #   depvar = unique(ans$effects$name),
  #   effect = c('Rate parameter period 1', 'Rate parameter period 2'),
  #   short = 'rate',
  #   estimate = ans$rate,
  #   se = ans$vrate
  # )

  #extract max conv rate
  tconv.max = tibble(
    depvar = unique(ans$effects$name),
    effect = 'Overall maximum convergence ratio',
    short = 'tconv.max',
    estimate = ans$tconv.max[[1]]
  )

  # combine and format
  effects |>
  # rate |> 
  #   bind_rows(effects) |> 
    bind_rows(tconv.max) |>
    mutate(
      z = estimate / se,
      z = ifelse(short %in% c('rate', 'tconv.max'), NA, z),
      p_value = 2 * pnorm(-abs(z)),
      stars = case_when(
        p_value < 0.001 ~ "***",
        p_value < 0.01  ~ "**",
        p_value < 0.05  ~ "*",
        p_value < 0.1   ~ "+",
        TRUE            ~ ""
      ),
      festimate = sprintf("%.3f%s", estimate, stars),
      fse = sprintf("(%.3f)", se),
      fse = ifelse(short == 'tconv.max', '', fse),
      z = round(z, 3),
      p_value = round(p_value, 4),
      conv_t = round(conv_t, 3)
    )  |>
    select(
      depvar, effect, short, interaction1, interaction2,
      estimate, se, z, p_value, stars, conv_t, festimate, fse
    )
}
res = list()
res[['ansm1']] = tidy_rsiena(ansm1)
res[['ansm2']] = tidy_rsiena(ansm2)
res[['ansm3']] = tidy_rsiena(ansm3)
res[['ansm4']] = tidy_rsiena(ansm4)
```

```{r}
res_table <- names(res) |>
  purrr::imap(function(name, i) {
    res[[i]] |>
      mutate(
        effect = str_remove(effect, "first: ") |> str_remove("last: ")
      ) |>
      select(depvar, effect, festimate, fse) |>
      rename_with(~ paste0(str_remove(name, 'ans'), "_", .), c(festimate, fse))
  }) |>
  purrr::reduce(full_join, by = join_by(depvar, effect)) |>
  arrange(depvar)

res_table |> writexl::write_xlsx(file.path(BASEDIR, 'results', 'res_table.xlsx'))
res_table
```


```{r}
setwd(BASEDIR)
```
