...
> Character string controlling which cleaned representation to write back into `functie`.  
> `"complete"` (default) uses the core academic rank (`position`);  
> `"simplified"` uses `position2`, which also includes distinctions such as visiting, external, emeritus, etc.
