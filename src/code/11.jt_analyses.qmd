...
resa$ego <- calculate_ego_results(wave1, node_data, mode = 'out')
...
exclude_junior_staff <- function(colnet) {
  ...
  out[['data']] <-  colnet[['data']] |>
    filter(!position_22 %in% c(
      'PhD Candidate', "Researcher or Lecturer", "Staff"
    ))
...
wave32 = colnet2[['nets']][[3]]

node_data2 = colnet2$data
resa$ego2 <- calculate_ego_results(wave12, node_data2, mode = 'all')
resa$ego2
```
...
```{r}
noiso1 = rowSums(wave1, na.rm = TRUE) + colSums(wave1, na.rm = TRUE) > 0
noiso2 = rowSums(wave2, na.rm = TRUE) + colSums(wave2, na.rm = TRUE) > 0
noiso = noiso1 | noiso2
...
resc$ego <- calculate_ego_results(wave1, node_data, mode = 'out')
...
node_data2 = colnet2$data
resc$ego2 <- calculate_ego_results(wave12, node_data2, mode = 'all')
resc$ego2
```
...
rese$ego <- calculate_ego_results(wave1, node_data, mode = 'out')
...
node_data2 = colnet2$data
rese$ego2 <- calculate_ego_results(wave12, node_data2, mode = 'all')
rese$ego2
```
